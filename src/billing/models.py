...
stripe.api_key = "sk_test_a9nujToOAsTsyvhbRmGGSuPR00rO6ijYaj"
...
class BillingProfileManager(models.Manager):
    def new_or_get(self, request):
        ...
        if user.is_authenticated:
            print('logged in user checkout; remember payment stuff')
            obj, created = self.model.objects.get_or_create(
                user=user, email=user.email)
        elif guest_email_id is not None:
            print('guest user checkout; auto reloads payment stuff')
...
class ChargeManager(models.Manager):
    def do(self, billing_profile, order_obj, card=None):  # Charge.objects.do()
        card_obj = card
        if card_obj is None:
            cards = billing_profile.card_set.filter(
                default=True)  # card_obj.billing_profile
            if cards.exists():
